...
今回は、この魅力的な`Tripo 2.0`の基本的な使い方や特徴的な機能の紹介、さらには「こんな場合はどうなるのか？」といった気になる内容の検証も行います。
...
3Dデータが生成されるのを少し間待ちます。生成が完了すると次の様に4つの結果が生成されます。それぞれを確認し、編集画面を開くために、目的の3Dデータの赤枠で囲った箇所をクリックします。
![](/images/turtle-20240929-tripo2-evo/prompt_usage_02.png)
*プロンプトによる3Dデータの生成結果*

以降の手順は、画像データで生成するのと同様の手順のため省略しますが、次のようにプロンプトからも3Dデータが生成され、3Dプリントすることができました。
