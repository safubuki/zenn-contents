...
以上で、便利な機能の説明は終了です。ここまでの章を読んでいただければ、`Tripo2.0`はすぐにでも使いこなせるのではないでしょうか？残りは、ちょっと気になる実験的な検証を行っています。もしよろしければ、引き続き見てみてください。

## 実験的検証

この章では、「こんな場合はどうなる？」というちょっとした疑問を検証しています。現状は2つだけですが、今後色々触って面白そうな内容あれば更新していきたいと思います。扱う内容は次の通りです。
...
以前のTripoですと、フォトリアルな画像は、平面的に出力されて3D形状が正確に再現されることはまずありませんでした。これが、新しいTripo2.0ではどのようになるのか確認してみました。検証に利用する画像は、私が以前書いたブログで色々と検証に使った画像を使用します。
...
3Dデータを生成する「Tripo2.0」基本および特徴的な機能とちょっと気になる内容を検証しました。
