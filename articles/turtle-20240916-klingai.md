...
- **リップシンク機能**
  - 既に作成済みの動画に対して、ナレーションや歌データをアップロードすると、その内容に合わせて口の動きが連動するというものです。より自然な感じで話したり、歌ったりしているようになります。
- **コミュニティ機能**
  - 作品を共有する機能が追加されました。おそらくトップ画面とかに表示されていくのだと思います。
- **APIプラットフォーム**
  - 企業や開発者などが、アプリケーションなどからアクセス可能なAPIが公開されました。Kling AIを使ったビジネスや開発が加速しそうです。
...
KLING 1.5, 1.0モデルどちらで作った動画でも構いませんので用意します。

1. 音声や音楽データを用意します。
動画と音声の長さは、できる限り合わせるようにします。また、私はVOICEPEAKというソフトの有償版を使用して合成音声を作成しました。
![](/images/turtle-20240916-klingai/gousei_soft.png =500x)
*VOICEPEAK（合成音声ソフト例）*
※ 無償版の合成音声ソフトは便利ですが、公開する場合、企業名やソフト名を表記する必要があるケースがあります。ご注意ください。

1. 手順1で用意した動画を開き、動画左下の`Match Mouth Type`ボタンをクリックします。
![](/images/turtle-20240916-klingai/match_type.png =500x)
*Match Mouth Typeのボタンの位置*
1. 少し待つと次のような音声データを登録する画面が表示されるので、ここに音声データをドラッグ&ドロップします。
![](/images/turtle-20240916-klingai/match_mouth_before.png =500x)
*音声データ登録前の*
1. 音声データのアップロードが完了すると次の画面が表示されるので、右下の`Lip Sync`ボタンを押します。
![](/images/turtle-20240916-klingai/match_mouth_after.png =500x)
*Match Mouth Typeのボタンの位置*
   消費するクレジットは以下の通りです。
   - 5秒動画 = 5クレジット
   - 10秒動画 = 10クレジット
1. 作成が完了すると次のような音声に口の動きが連動した動画が生成されます。

https://www.youtube.com/watch?v=jH7DJMd9tVY

また、こちらも作成してみました。もしよろしければどうぞ。

https://www.youtube.com/watch?v=fYUqur4M_V4

### コミュニティ機能

コミュニティ機能は自身が作成した生成物を、Klingのコミュニティに共有する機能です。掲載されるには審査が必要が必要とのことです。私も審査に通るかどうかよくわかりませんが、上記の動画を投稿してみました。

1. TOP画面からCommunityを選択します。
![](/images/turtle-20240916-klingai/comm_01.png =500x)
*Communityの位置*
2. Publishボタンを押下します。
![](/images/turtle-20240916-klingai/comm_02.png =500x)
*Publishボタンの位置*
3. 必要事項を入力して、Publishします。
動画をアップロードしないとPublishボタンはグレーアウトのままで押せないです。
