...
私は2024年1月から使い始めましたが、驚くほど便利です。
GitHub Copilotは、自然言語による指示やコードの一部を入力するだけで、目的のコードを生成してくれます。さらに、問題点の検出や修正、コメント、テストの作成など、あらゆることをこなしてくれます。そして、エディタ上ですぐに反映できるので、開発をとてもスムーズに行うことができます。
...
このブログでは、GitHub Copilotが初めてという方のために**使い方の基本**を実例と図を交えながら丁寧に説明し、**品質向上のための手法**など少し発展した内容も紹介します。このブログを読んで、あなたもぜひGitHub Copilotの魅力を感じてみてください👍
...
- **開発効率の向上**
  ...
  - 日々の開発作業をよりスムーズに進めることができるます。
- **品質向上のための手法**
  - 静的解析ツールとの組み合わせにより、コードの品質を向上させる方法が得られます。
  - バグの発見と修正に役立つテクニック得ることができます。
- **注意点の理解**
  - GitHub Copilotの使用時に気をつけるべきポイントを把握できます。

## GitHub Copilotとは

GitHub Copilotは、AIがコードを自動生成し、開発者の生産性を向上させる画期的なツールです。それでは、GitHub Copilotの機能の一覧と利用することによるメリットを見ていきましょう。

### 機能一覧

GitHub Copilotには、多くの機能がありますが、その中でも代表的な機能の一部を次の図に示します。

![func_list](/images/turtle-20240223-gitcopilot/func_list.png)
*GitHub Copilot機能一覧(代表例)*

コーディングに必要な各種機能が組み込まれており、AIの力を借りてスムーズかつ効率的に進めることができます。
何だかワクワクしてきませんか？これらの機能を使いたくなってきたでしょうか。
...
GitHub Copilotは、**有償**（**学生ほか一部対象者は無料**）のツールです。利用するためには、アカウントの作成や契約、エディタの拡張機能インストールが必要です。これらの方法について説明します。

なお、既に導入済みの方や、企業などの組織で利用される方（組織で提供される利用手順書等を参照いただくのが確実です）は、**この章をスキップ**して構いません。

この章は、これから使ってみようという**個人利用者を対象**とします。
...
これらの内容を理解することで、GitHub Copilotの基本機能をマスターできます。
...
5. Acceptで処理を受け入れて確定し、ファイルを保存します。
...
なお、コードの提案が期待する内容ではなかった場合、**Ctrl + Enter キー**を入力することで、別の提案の一覧を表示することができます。期待する提案が一覧にあれば **Accept suggestion X ボタン**を押して、選択した提案を反映します。
...
このように、コード生成をサポートしてもらったり、困ったことを質問して解決したいときなどに、チャット機能はエディタ上で頼れる存在です。

### ショートカットキー

GitHub Copilotでは、次のショートカットキーを使うことができます。なお、詳細な使い方は、既に本章の機能①～③の説明でgif動画を交えて触れていますので、そちらをご参照ください。ショートカットキーの一覧と内容を示します。
...
GitHub Copilotには、目的の機能を簡単に呼び出し、素早く実行するために**スラッシュコマンド**があります。チャット欄に`/fix` や `/doc` などを入力するだけで、コード修正をしたり、関数の説明コメントを追加したりすることができます。本章では、コマンドの一覧を示し、よく使うものについてはさらに詳細な使い方を説明を行います。
...
/explain は、コードの意味や動作を分かりやすく説明するコマンドです。自分でプログラムを作成するときはあまり必要ないかもしれませんが、他の人が作成したプログラムを読む際、コメントがない、処理のロジックがわからないなどの場合に便利なコマンドです。`/explain`のみを入力すると、**英語で説明**が表示されますが、`/explain 日本語で`と入力すると、**日本語で説明**が表示されます。
...
お疲れさまでした！以上が、使い方の基本になります。図解で初めての方にも分かりやすく説明したつもりですが、もし分かりにくい内容がありましたら、コメント欄にご記入いただければと思います🙇‍♂️

それでは、あと少しお付き合いください。

## 気をつける点

ここまで、Github Copilotのメリットや魅力的な機能を紹介してきましたが、AIを使っている以上注意すべき点もいくつかあります。ここでは、Github Copilotを使用時に気をつけるべきポイントを説明します。

### 推奨言語以外では期待する結果が得られない可能性がある

GitHub Copilotの公式サイトにも記載がありますが、適している言語は次の通りです。
...
理由として、多くのプロジェクトや開発者に利用されており、Copilotの学習にも用いられる割合が高いためです。これらの言語以外でも、提案はされると思いますが、期待する精度の結果が得られない可能性があります。

(参考URL) [GitHub Copilot の概要 (公式)](https://docs.github.com/ja/copilot/using-github-copilot/getting-started-with-github-copilot)

### 古いライブラリやモジュール、APIが提案される

GitHub Copilotは、過去のコードを元に学習しているため、場合によっては古い情報に基づいた提案が行われることがあります。
以前、PDFをテキストに変換するアプリをGitHub Copilotに生成してもらい、試しに動作させてみたところ次のようなエラーが出力されました。

> PyPDF2.errors.DeprecationErrorreader.numPages is deprecated and was removed in PyPDF2 3.0.0. Use len(reader.pages) instead.

GitHub Copilotが提案したコードは、古いバージョンのAPIを使用したものでした。
このようなことが発生する点に注意してください。

なお、エラー出力された場合、手動で修正してもよいですが、Copilotに任せて`/fix`コマンドに**エラー情報を指示文として与えて修正**する方法も良さそうです。

### あくまでもCopilot（副操縦士）である

GitHub Copilotは、あくまでもCopilot（＝副操縦士）であるという点は、留意する必要があります。上記で示したような例からも分かるようにAIは、完ぺきではありません。全てを委ねて、何でも任せて何もしなくてよいというものではありません。

提案されたコードについては、自身の目で問題がないか確認を行い、必要に応じて修正することは大切です。また、プログラミングに対する基本的な知識、スキルと学習は必要です。

これらのポイントをおさえて、GitHub Copilotを活用してください。
...
始めたばかりの方や、まずは基本をおさえたいという方は、ここまでの内容でも十分です👍一旦、復習のために、今までの内容を振り返りながら、実際に触ってみると良いと思います。

次の章では少し、発展的な内容を説明します。
引き続き興味がある方は、ご覧いただけると嬉しいです。

## 品質を高める方法

GitHub Copilotは、
コード生成や問題の修正、テストコードの作成など効率的な

効率的にコーディングすることができ、バグの素早い検出や修正が可能になることで、プログラムの品質を向上します。しかし、AIがこれだけでは、品質を保証することは難しいです。なぜなら、AIである以上誤った提案をすることがありますし、また結果に一貫性が無い場合があるからです。


GitHub Copilotは、効率的にコーディングすることができ、バグの素早い検出や修正が可能になることで、プログラムの品質を向上します。しかし、AIがこれだけでは、品質を保証することは難しいです。なぜなら、AIである以上誤った提案をすることがありますし、また結果に一貫性が無い場合があるからです。
...
これらの手法は、コードの一貫性を保ち、潜在的なエラーやバグを事前に検出するのに役立ちます。実際の活用方法については、私が書いた記事にまとめておりますので、もしよろしければ見てみてください。

https://zenn.dev/safubuki/articles/turtle-20240215-pymind

### 従来手法と組み合わせる

GitHub Copilotは、従来から品質を高めるために使われてきたツールや仕組みととても相性が良いです。組み合わせて使うことで、さらに便利に使うことができます。
...
GitHub Copilotは、従来の手法でうまく補いつつ、便利に組み合わせることで、より品質の高いプログラムを作成することができます。ぜひ、この方法を活用してみてください。
