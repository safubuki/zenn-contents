...
GitHub Copilotの操作は、GUI(マウスなどで操作する画面)からでも行うことができます。しかし、コーディングをする際は、**効率的に作業を行うため**に、キーボードからのショートカットキーを覚えておくと便利です。

#### 一覧

以下に、ショートカットキーの一覧を示します。
| ショートカット       | 内容                                                                | よく使う(★)  |
|---------------|-------------------------------------------------------------------|--------|
| Ctrl + I (アイ) | **インラインチャット機能を起動**<br/>コードを書いているファイルの中で、<br/>チャットを起動し、コードの生成や修正、質問できる | ★★★    |
| TAB           | **コード提案を受け入れる**<br/>コードを書いているときに、<br/>提案してくるコードの案を受け入れて確定する           | ★★★    |
| ESC           | **コード提案を拒否する**<br/>コードを書いているときに、<br/>提案してくるコードの案を拒否する                 | ★★★    |
| Ctrl + Enter  | **コード提案の一覧を表示する**<br/>コードを書いているときに、<br/>提案してくるコードの別案の一覧を表示する         | ★★☆    |
| Alt + ]       | **次の提案を表示する**                                                         | ★☆☆（※） |
| Alt + [       | **以前の提案を表示する**                                                        | ★☆☆（※） |

(参考URL) [GitHub Copilot の概要 (公式)](https://docs.github.com/ja/copilot/using-github-copilot/getting-started-with-github-copilot)
(※) なお「次/以前の提案を表示する」は、私の環境では、正常に動作しませんでした。
...
GitHub Copilotのチャット機能で使用する`/fix` や `/explain` など、/(スラッシュ)コマンドについて説明します。これらのコマンドは、コードを修正したい、コードの説明をしてほしいというシーンで**具体的に目的を示して、指示する**際に使用します。これらのコマンドは使わなくても、AIが指示の内容を解釈して、目的の動作をしてくれますが、まれに**関数の修正の指示をしているのに、関数の説明をするケース**があります。これらのコマンドは、**指示の意図を明確にし、効率的に利用するために必要**ですので、ぜひ覚えておきましょう。一覧を示して、そのあとに具体的な使い方を説明します。
