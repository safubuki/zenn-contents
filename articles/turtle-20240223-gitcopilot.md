...
#selectionは、アクティブなエディタのカーソル選択した範囲を示すための変数です。アクティブなエディタ上で、説明や修正したいコードをカーソル選択した上で、`#selection 処理内容を説明してください`や`#selection 処理を簡略化してください` と入力することで、カーソル選択した箇所のコードについてのみ説明や修正を行うことができます。

★GIF動画

なお、個人の使用感ですが、300行程度のコードは省略せずに説明等を行ってくれます。しかし、それ以上の重厚長大なコードになると、説明が一部省略されたり、適切な回答を得られないケースがあります。うんざりするような長いコードでも、一度にまとめて選択するのではなく、数関数ずつなど細かく分けて選択し、回答を得るようにしてください。

##### #terminalSelection

#terminalSelectionは、アクティブなターミナルのカーソル選択した範囲を示すための変数です。ターミナルとは、PCの操作やプログラムを実行するためのコマンドツールで、VSCodeの場合`Ctrl ＋ @`で起動します。このterminalSelectionが効果を発揮するのは、プログラムを実行して、何らかエラーが発生したときです。ターミナルに表示されたエラーをカーソル選択し`#terminalSelection エラーの原因と対策を教えてください`と入力します。すると、エラーがなぜ発生したのか、エラーを取り除くためにどのような対策をしたらよいか、教えてくれます。

★GIF動画

回答が100%の内容でなかったとしても、解決の糸口をすぐに得ることができます。従来はインターネットでエラーコードを検索し、サイトによって異なる記述を比較しながら、試行錯誤して解決していたと思います。それに比べると圧倒的な速さで次々エラーをつぶしていけるので、本当にお勧めです。

##### #file

#fileは、ワークスペース内の選択したファイルを示すための変数です。ファイル概要やファイル同士の関係性把握など、ファイルそのものについて回答を得たいとき便利です。`#file:sample.py ファイルの概要と関数の一覧を示してください`や`#file:kame.py #file:turtle.py これらのファイルの関係性を示してください`のように入力します。

★GIF動画

#selectionの時に説明しましたが、こちらもあまりにも巨大なファイルですと、一部回答が省略されることがあります。その場合は、ファイルを開いて#selectionで、少しずつ回答を得るようにしてください。
...
- **2024/03/04**
  説明に用いている一部の画像やgif動画の文字が見づらかったため、画像サイズを大きくしました。
