...
GitHub Copilotは、有料のツール（**学生ほか一部対象者は無料**）です。利用するためには、アカウントの作成や月または年単位の契約、エディタの拡張機能インストールが必要です。これらの方法について説明します。

なお、既に導入済みの方や、企業などの組織で利用される方（組織で提供される利用手順書等を参照いただくのが確実です）は、この章をスキップして構いません。

この章は、これから個人で使ってみようという方を対象にします。
対象者が限られるため、デフォルトでは、説明の表示を閉じています。
必要に応じて「GitHub Copilotの導入方法の説明」を各項目をクリックして、説明を開いてください。
::::message
**GitHub Copilotの導入方法の説明**
:::details アカウント作成（クリックで開く）

### アカウント作成

GitHub Copilotの利用には、**GitHub**のアカウント作成が必要になります。
ちなみに、GitHubは、web上にプログラムコードを保存し、バージョン管理やメンバで共有するためのサービスです。こちらも開発には無くてはならないものですから、この機会に個人のGitHubアカウントを作成しましょう。
...
　 個人が無料のアカウントを複数持つことはライセンス違反となるためです。
　 個人用、業務用それぞれ分けて使う場合は問題ないようです。
　 (参考)複数のGitHubアカウントを保持していいの？GitHubサポートに問い合わせてみた
　 [https://www.timedia.co.jp/tech/20221025-tech/](https://www.timedia.co.jp/tech/20221025-tech/)
:::

:::details 有料プラン契約（クリックで開く）

### 有料プラン契約

GitHub Copilotの利用には料金がかかります。


:::

:::details インストール（クリックで開く）
