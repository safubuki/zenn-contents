...
あとは、画像生成のためのプロンプト（指示文）を入力するだけ、画像が生成されます。

#### TripoSRに特化した画像生成のコツ

通常、画像生成AIを利用するときは、とても凝ったプロンプトを考えがちですが、TripoSR向けの画像を生成する場合は、それほど凝る必要はありません。ポイントは次の2点です。

- シンプルな背景にする
- 3D形状を把握しやすい画像を生成する

TripoSRは、背景にノイズとなるものが多い複雑な背景は失敗しやすいので、シンプルな背景にします。また、平面的な画像から3Dデータを作成することも苦手ですから、できる限り3D形状を把握しやすい画像を生成することもポイントです。具体的に示します。
...
```
どちらのプロンプトも、シンプルな背景と3D形状を把握しやすくするためのワードが散りばめられています。


どちらも、シンプルな背景と「3DCG」「カートゥーン」「陰影」「3D形状がよくわかる」など、3D形状を把握しやすい画像の生成を意識したプロンプトになっています。このようなプロンプトで生成した画像は、3Dデータを

### ●3Dモデル生成AI（TripoSR）

TripoSRで、画像生成AIで作成した
