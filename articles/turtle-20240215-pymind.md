...
次の図に示すような各種便利な機能を提供する**6つの拡張機能**をVSCodeにインストールして使います。まずこれらをインストールしてください。似たような名前の拡張機能もありますので、拡張機能の名前や作者の名前を正しく確認して、作業を行ってください。
...
2. **次のソースコードをコピーして、`sample.py`にペーストします。**
...
yapf、isortの働きによりコードの見通しが良くなりました。この他にも1行あたりの文字数が99文字を超えていた場合、範囲内に収まるように適切に改行します。
...
4. **いくつかの指摘が残っていますので、それを修正していきます。**
   ここでは代表的なものを見ていきます。
   まず、1行目の`import math`に指摘があります。
   マウスカーソルを当てると次のような表示が出てきます。
   ![shiteki](/images/turtle-20240215-pymind/shiteki.png =550x)
   *拡張機能による指摘*
   どうやら、mathモジュール使われておらず、不要ということのようです。今回はこのメッセージの一文だけで指摘が分かりましたが、場合によっては指摘内容が理解できないことがあります。その時は、赤線で引いた箇所の青い文字をクリックしてみてください。すると、次のような問題事例と正しいコードを示したWebページのリンクに移動します。
   ![pylint_page](/images/turtle-20240215-pymind/pylint_page.png =550x)
   *Pylintドキュメントサイト*
   このページには問題のあるコードと正しいコードのサンプルを交えた解説がありますので、指摘内容の理解と改善に役立ちます。そして、さらに便利な機能がQuick Fixです。以下の赤線をクリックします。
