...
WebUIには、Indexリストから欲しい拡張機能を探して、簡単にインストールできる仕組みがあります。私は、自分が作ったものを他の人とも共有できたらいいなと思いました。そこで、**WebUI Indexリストへの登録**もしてみました。
...
Gradioを使うとWebアプリケーションの作成が簡単です。Pythonのモジュールで、関数と入出力の形式を指定するだけで、インタラクティブ（双方向）なUIができる優れものです。ただ、それゆえに表現できる内容は限られます。

Gradioでどのようなことができるかは、公式サイトのドキュメントを見てみてください。

https://www.gradio.app/docs/interface

その他、皆さんが使い慣れている拡張機能を参考にするのも一つの手です。既にいくつかインストールしているなら、extensionsフォルダで見ることができます。
...
- **ソースコード**
:::message
以下のリンク先のコードを元に説明しますので、確認してください。
:::
...
拡張機能の作成イメージはつかめましたか？
準備で作成した開発用のpyファイルに自身でコードを書いて、実際の動きを確かめてみてください👍
...
なお、これらのファイルも必要なものが完成したら、GitHubにコミット、プッシュしてみてください。
...
7. プルリクエストを行います。
descriptionに自身のギットのURLなど簡単な情報を記載して、Checlistにチェックを入れます。チェックはカッコにxを書くことで入れることができます。
