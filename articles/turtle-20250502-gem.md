...
当初、私はこの機能を十分に活用できていませんでした。しかし、AIモデルが「**Gemini 3 Pro**」などに進化し、機能が洗練された現在、Gemは私にとって無くてはならない存在へと変わりました。

今では、最新の検索結果を反映した回答を得たり、音楽AIの歌詞から動画用プロンプトを一括生成してもらったりと、その活用範囲は実用系から音楽、画像、動画に至るあらゆるクリエイティブへと広がり続けています。既に150以上のGemを作成し、日々活用しています。

この記事では、そんなGeminiのGemについて、基本的な使い方から、試行錯誤して編み出したカスタマイズのステップ、そして実際に役立つGemの実例まで、分かりやすく解説します。
...
* **専門性の付与**: 特定の知識や視点（例えば、特定の業界知識や文体など）を指示に組み込むことで、より専門的な応答を引き出すことが可能です。
...
Gemを作成するには、まずGeminiの画面左側メニューから「Gem」を開き、「+ Gemを作成」ボタンをクリックします。
...
Gemの編集画面（新UI）が開きます。ここでは以下の項目を設定できます。
*   **名前**: Gemに名前をつけます（必須）。
*   **説明**: Gem一覧に表示される短い紹介文です。自分用のメモとして書いておくと便利です（空欄でもOK）。
*   **カスタム指示**: AIへの命令を書くメインの場所です。ここにGemにさせたいことを入力します。
*   **デフォルト ツール**: Gemが使用する標準機能を設定できます。
*   **知識**: 独自の資料をGemに読み込ませることができます。現在はファイルだけでなく画像も追加できるようになっています（本記事では詳細は割愛します）。

![](/images/turtle-20250502-gem/gem_ui_new.png =600x)
*Gem作成メイン画面（新UI）*

「デフォルトツール」は、基本的には「デフォルトツールがありません」のままで問題ありません。
もし、このGemを画像生成やDeep Research専用として使いたい場合は、リストから該当するツール（「画像を作成」など）を選んでおくと、毎回モデルを指定する手間が省けてスムーズです。

![](/images/turtle-20250502-gem/gem_main_def.png =400x)
*デフォルトツールの選択（基本は「なし」でOK）*

それでは、「カスタム指示」の欄に、Gemにさせたいことを自分の言葉で入力してみましょう。最初は完璧を目指さず、思いつくままに書いてみてください。
...
Geminiのチャット画面を開き、以下のように入力します。
※相談する際は**Gemini 3 Flash（思考モード）**や**Gemini 3 Pro**のような高度な推論モデルをオススメします。
```
...
**【タイトル：企業ロゴイラストメーカー】**
以下の文章をGemのカスタム指示欄に貼り付けてご使用下さい。
また、このGemは直接画像生成できます。AIモデルは**Gemini 3 Flash（思考モード）**または**Gemini 3 Pro（Proモード）**が推奨ですが、**Gemini 3 Flash（高速モード）**でも生成は可能です。

> [!NOTE]
> 高性能な画像生成モデル「NanoBananaPro」が利用可能で、非常に高品質なロゴが生成できるためです。
```
## 目的と目標:
...
- **2025/12/20**
 記事全体の可読性向上、Gemini 3（Flash/Pro）への対応、新UI（デフォルトツール等）への対応、およびGemマネージャーの表記を「Gem」に変更
