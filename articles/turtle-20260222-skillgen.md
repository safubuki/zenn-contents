...
```bash
cd .github/skills
git clone https://github.com/YKame/skills-generator.git
```
...
ジェネレーターの「脳」となるファイルです。発火条件や4つの動作モード（作成・改善・監査・翻訳）が定義されています。

:::details SKILL.md の中身（クリックして開く）
```markdown
---
...
### 2. テンプレート（`assets/skill-template.md`）

このジェネレーターが出力する「スキルのひな型」です。
...
### 3. ルールブック（`references/best-practices.md`）

AIに「スキルの名前の付け方」や「発火させるコツ」を教え込むデータベースです。
