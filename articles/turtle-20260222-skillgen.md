...
![ワークスペースで活躍するskillsたち](/images/turtle-20260222-skillgen/active-skills-list.png)  
...
![ブログ構成図](/images/turtle-20260222-skillgen/blog_structure.png)  
*ブログ構成図*
...
ジェネレーターの中心となるファイルです。ここでは「スキルを作る」という単一の処理だけでなく、ユーザーの要望に応じて**4つのモード**を使い分けるように指示が定義されています。
...
今回、皆さんに私のGitHubからスキルリポジトリをダウンロードしていただきました。ここで、ネット上に公開されている第三者のソースコードを利用する前に知っておくべき**重要なセキュリティのお話**をします。
...
description: フロントエンドプロジェクトのREADMEを自動生成するスキル。package.jsonを読み込み、依存ライブラリ一覧・セットアップ手順・スクリプト説明・必要なNode.jsバージョンを解析して実用的なドキュメントを生成する。「READMEを作って」「READMEを生成」「README作成」「ドキュメントを作って」「セットアップ手順を書いて」「依存関係を整理」「package.jsonからREADME」などで発火。
---

# README Generator — フロントエンド README 自動生成スキル
...
> `package.json` を解析し、セットアップ手順・依存ライブラリ一覧を含む README を自動生成します。

## When to Use

- 新しいフロントエンドプロジェクトに README がないとき
- 既存の README が古くなって刷新したいとき
- `package.json` の内容を元に依存ライブラリ一覧を整理したいとき
- セットアップ手順をドキュメント化したいとき
- OSS 公開・チーム共有に向けてプロジェクト説明を整備したいとき

## 概要

`package.json` を読み込んで、フレームワーク・ライブラリ・スクリプト・エンジン要件を解析し、  
すぐに使える **README.md** を自動生成します。  
React / Vue / Next.js / Nuxt / Vite / Svelte などの主要フレームワークを自動判定し、  
フレームワークに合ったセットアップ手順を出力します。

---

## 手順

### Step 1: package.json の読み込みと解析

プロジェクトルートの `package.json` を読み込み、以下の情報を抽出する：

| 抽出項目 | フィールド | 用途 |
|---------|-----------|------|
| プロジェクト名 | `name` | README タイトル |
| バージョン | `version` | バッジ・ヘッダー |
| 説明 | `description` | プロジェクト概要 |
| エントリーポイント | `main` / `module` | 技術説明 |
| スクリプト | `scripts` | セットアップ・実行コマンド一覧 |
| 依存ライブラリ | `dependencies` | 本番依存ライブラリ一覧 |
| 開発依存ライブラリ | `devDependencies` | 開発ツール一覧 |
| Node.js 要件 | `engines.node` | 動作環境 |
| パッケージマネージャー | `packageManager` / lockfile | インストール手順の分岐 |
| ライセンス | `license` | ライセンスセクション |

> 💡 `package.json` が見つからない場合は、ユーザーにパスを確認する。  
> モノレポ構成の場合は対象パッケージを確認する。

### Step 2: フレームワーク・技術スタックの自動判定

`dependencies` と `devDependencies` から以下のルールでフレームワークを判定する：

**フレームワーク判定ルール（優先順位順）**

| フレームワーク | 判定キー | セットアップ特記事項 |
|--------------|---------|-------------------|
| Next.js | `next` | `next dev` で起動 |
| Nuxt | `nuxt` | `nuxi dev` |
| Remix | `@remix-run/react` | `remix dev` |
| Astro | `astro` | `astro dev` |
| SvelteKit | `@sveltejs/kit` | `vite dev` |
| Vue + Vite | `vue` + `vite` | `vite` で起動 |
| React + Vite | `react` + `vite` | `vite` で起動 |
| React (CRA) | `react-scripts` | `react-scripts start` |
| Vite (その他) | `vite` | `vite` で起動 |
| Angular | `@angular/core` | `ng serve` |
| Svelte | `svelte` | `rollup` / `vite` を確認 |

**パッケージマネージャー判定ルール**

| マネージャー | 判定根拠 |
|------------|---------|
| pnpm | `packageManager: "pnpm@..."` または `pnpm-lock.yaml` |
| yarn | `packageManager: "yarn@..."` または `yarn.lock` |
| bun | `packageManager: "bun@..."` または `bun.lockb` |
| npm | 上記以外（デフォルト） |

### Step 3: 依存ライブラリの分類・整理

各ライブラリに役割ラベルを付与してテーブル化する。

**ライブラリ役割ラベル一覧**

| ラベル | 対象ライブラリ例 |
|--------|----------------|
| UI | `react`, `vue`, `svelte`, `@angular/core` |
| Router | `react-router-dom`, `vue-router`, `@tanstack/router` |
| Store | `zustand`, `pinia`, `jotai`, `redux`, `recoil` |
| Fetch | `@tanstack/react-query`, `swr`, `axios`, `ky`, `ofetch` |
| Style | `tailwindcss`, `styled-components`, `@emotion/*`, `sass`, `unocss` |
| UI Lib | `@radix-ui/*`, `shadcn-ui`, `@mui/*`, `antd`, `@chakra-ui/*`, `daisyui` |
| Form | `react-hook-form`, `formik`, `vee-validate`, `zod`, `yup`, `valibot` |
| Anim | `framer-motion`, `motion`, `@vueuse/motion`, `gsap`, `anime` |
| Auth | `next-auth`, `@auth/core`, `firebase`, `@supabase/supabase-js` |
| Test | `vitest`, `jest`, `@testing-library/*`, `playwright`, `cypress` |
| Lint | `eslint`, `prettier`, `biome` |
| Build | `vite`, `webpack`, `rollup`, `esbuild`, `turbopack` |
| Types | `typescript`, `@types/*` |

> 💡 ライブラリが多い場合（20個超）は、セクション分けしてラベリングする。

### Step 4: README.md の生成

📄 **[assets/readme-template.md](assets/readme-template.md)** に従って生成する。

**生成ルール**

1. `description` が空の場合はプロジェクト名からの推測か、ユーザーに確認する
2. `scripts` に定義されていないコマンドは記載しない
3. バッジは `shields.io` 形式で自動生成する
4. 環境変数が必要と推測される場合は `.env` セクションを追加する
5. `engines.node` が未指定の場合は「推奨: Node.js 18 以上」と記載する

### Step 5: 出力と確認

生成した README.md をプレビュー表示し、ユーザーに以下を確認する：
...
確認後、ワークスペースルートに `README.md` として保存する。

---

## 参照ドキュメント

- [assets/readme-template.md](assets/readme-template.md) — README 出力テンプレート
- [references/package-json-fields.md](references/package-json-fields.md) — package.json判定ロジック
- [references/framework-setup-guide.md](references/framework-setup-guide.md) — フレームワーク別セットアップコマンド
```
:::
...
![Remotion Skills Ecosystem Map](/images/turtle-20260222-skillgen/remotion-ecosystem-map.png)
...
ちなみに、次の動画は、このスキル群を使って初めてプロンプトから自動編集して作成した動画です。この時は静止画のみでしたが、今はスキルも育って動画なども扱えるようになっています。

https://youtu.be/YfZngqjH9wA

## まとめ

「Gemを作るGem」「GPTを作るGPT」に続き、今回はIDE内で活躍する「Agent Skillsを作るスキル（**Skills Generator**）」の構造とその導入手順について解説しました。

本記事を通して、Agent Skillsが単なる「長いプロンプト」ではなく、テンプレート（`assets/`）や専門知識のデータベース（`references/`）を組み合わせた**実践的なエコシステム**になり得ることが伝わったのではないでしょうか。

また、今回は以下の重要な実践的ノウハウも合わせてご紹介しました。
* 新たなスキルはシンプルな要件（Generate）から始め、**Improveモード**で段階的に社内ルールに合わせて育てていくアプローチ
* 便利さの裏に潜む「プロンプトインジェクション」や「データ流出」といった**野良スキルのセキュリティリスク**への警戒
* 自衛の手段としての**Audit（監査）モード**の活用と、最終的な目視確認の重要性

AIエージェントは、ただ与えられた一般的なコードを書くだけでなく、「**ご自身のプロジェクトに最適化された作法や知識を教え込む**」ことで本来の有用性を発揮します。
手作業で一つ一つのルールや設定ファイルを書くのが面倒だと感じたら、まずはこの記事の手順に沿って「専属のスキルアーキテクト」を手元にクローンし、配属してみてください。

これさえあれば、単なるプロンプトを超えた「プロジェクト専用の自動化・支援ツール」を安全かつ効率的に量産し、日々の開発体験をより良くすることができるはずです！

最後までお読みいただき、ありがとうございました。
もし、この『Skills Generator』を使ってみた感想や、新しく作った便利なAgent Skillsなどがありましたら、ぜひコメントでお知らせください。🐢
...
決まりきった定型処理はスクリプトに任せ、柔軟な判断が必要な部分はAIに委ねるという合理的な役割分担について紹介しています。毎回AIに一から考えさせる手間を省き、処理の高速化やトークン節約を実現する仕組みがわかります。

@[card](https://zenn.dev/safubuki/articles/turtle-20260705-scripted-skillgen)
