...
作成した `skills` ディレクトリの直下に、以下のように `skills-generator` フォルダとその中身（`assets` など）を作成していきます。（※以降の解説では、パス表記の例として GitHub Copilot の `.github/skills` を使用します。ご自身の環境に合わせて読み替えてください）
...
次に、ジェネレーターの「脳」となるメインファイルを作成します。以下の内容をコピーして `.github/skills/skills-generator/SKILL.md` に保存してください。
...
このジェネレーターが出力する「スキルのひな型」です。`.github/skills/skills-generator/assets/skill-template.md` に保存します。
...
ここでAIに「スキルの名前の付け方」や「発火させるコツ」を教え込みます。`.github/skills/skills-generator/references/best-practices.md` に保存します。
...
最後に、AIが構造を模倣するための「理想的な完成品」のサンプルです。`.github/skills/skills-generator/examples/example-skill/SKILL.md` に保存します。
