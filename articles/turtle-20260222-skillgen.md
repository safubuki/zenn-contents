...
本記事では、このAgent Skillsの利点から、あなただけのスキルを量産するための便利なメタスキル「**Skills Generator**」の全貌と完全な作り方までを公開します。
...
端的に言うと、**「常に適用される基本ルール」か「必要な時だけ呼ばれる専門スキル（マニュアル＋道具箱）」か**の違いです。
...
このジェネレーターが出力する「スキルのひな型」です。`.agent/skills/skills-generator/assets/skill-template.md` に保存します。
...
ここでAIに「スキルの名前の付け方」や「発火させるコツ」を教え込みます。`.agent/skills/skills-generator/references/best-practices.md` に保存します。
...
作成した `skills-generator` を実際に試してみましょう。
...
すると、AIはただの文章ではなく、私たちが先ほど用意した `best-practices.md` や `skill-template.md` を裏で読み込み、以下のような**しっかり構造化されたスキルセット**をすぐに作成・提案してくれます。
